...
def spider_atleti_master_con_tempo():
    ...
    for gara in lista_gare:
        ...
        # 🎯 CONTROLLO COMITATO
        if not nome_comitato or nome_comitato == 'Generico':
            print(f"⚠️ Salto la gara '{nome_g}': Comitato assente o 'Generico'.")
            continue
            
        slug_sito = COMITATI_URL.get(nome_comitato)
        if not slug_sito:
            print(f"⚠️ Salto la gara '{nome_g}': Comitato '{nome_comitato}' non trovato nel dizionario.")
            continue
        
        stagione_fisi = calcola_stagione_fisi(data_g)
        print(f"\n🟢 Analizzo: {nome_g} a {luogo_g} ({nome_comitato} | Data: {data_g} | Stagione: {stagione_fisi})")
        
        # 🔗 URL DINAMICO CON LO SLUG DELLA REGIONE SCELTA
        url_comp = f"https://comitati.fisi.org/{slug_sito}/competizione/?idComp={id_comp}&d={stagione_fisi}"
        print(f"🔗 Cerco competizione su: {url_comp}")
        ...
        try:
            ...
            if not id_sottogare:
                print("   ⏩ Nessuna classifica trovata a questo indirizzo.")
                continue

            for id_g in id_sottogare:
                # 🔗 URL DINAMICO ANCHE PER LA SINGOLA GARA
                ...
                # ESTRAZIONE CATEGORIA E SPECIALITÀ (Lasciato intatto dal tuo codice)
                ...
                # ESTRAZIONE ATLETI E TEMPI (Lasciato intatto dal tuo codice)
                ...
                while i < len(testi_atleti) - 7:
                    if testi_atleti[i].isdigit() and testi_atleti[i+1].isdigit() and len(testi_atleti[i+1]) >= 3:
                        batch_atleti.append({
                            ...
                            "luogo": luogo_g,
                            "data_gara": data_g,
                            "comitato": nome_comitato # 🎯 AGGIUNTO IL COMITATO PER FLUTTER!
                        })
                        i += 8
                    else:
                        i += 1
                ...
                if batch_atleti:
                    ...
                    print(f"   ✅ Salvati {len(batch_atleti)} atleti per la gara ID: {id_g}")
                
                time.sleep(0.5) # Pausa di rispetto per il server FISI

        except Exception as e:
            print(f"   ❌ Errore durante l'elaborazione dell'evento {id_comp}: {e}")
